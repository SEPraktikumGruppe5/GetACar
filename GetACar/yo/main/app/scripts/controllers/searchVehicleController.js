angular.module('mainApp')
    .controller('SearchVehicleController', ['$scope', 'GeoService', '$timeout', '$log',
        function ($scope, GeoService, $timeout, $log) {
            $scope.cities = function (namePart) {
                return GeoService.cities(namePart, 10, function (successResponse) {
                    var cities;
                    cities = [];
                    var i;
                    for (i in successResponse.data.cities) {
                        if (successResponse.data.cities.hasOwnProperty(i)) {
